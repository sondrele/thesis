...
Primarily the system is more restricted than conventional systems especially when it comes to storage.
...
If the argument is an archive the program will iterate through all the objects in the archive an report the size for each individual object file.
When reporting the size of the archives the sizes for all the objects files have been accumulated.

\subsection{Parameters}
\label{sec:size:params}
The various object files where compiled by setting the compiler optimization flags in the \file{Cargo.toml} file.
...
The \gls{rel} libraries, presented in \gls{rel}, all contain {\rust} code only, while the \lib{cmsis} and \lib{startup} libraries contain {\rust} binding code in addition to the actual {\C} implementation code compiled with \prog{gcc}.
The same compiler options from \autoref{sec:size:params} are passed to the {\C} compiler as well.
...
The \prog{arm-none-eabi-size} makes the distinction between three of the {\elf} segment.
...
This is because there is not any static allocation in the \gls{rel} {\rust} libraries, in fact the only static allocation is of 8B in the \textbf{data} section of the \lib{startup} library.
