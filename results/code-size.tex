...
When putting an embedded system into production, the number of microprocessors are usually large.
...
Here, only the size for the \elfsec{.text} segment is shown in the graphs, as the \elfsec{.data} and \elfsec{.bss} are mostly non-existing.
...
This increase of 500B is not due to the {\rust} code of the \lib{startup} library, but the \gls{cmsis} Cortex-M3 System Layer provided by Silicon Labs, which is written in {\C}.
...
This subsection considers the code size of actual runnable programs.
