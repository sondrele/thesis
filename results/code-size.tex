...
The programs were generated setting the parameters in the \file{Cargo.toml} file.
...
The \gls{rel} libraries are all {\rust} code, while the \lib{cmsis} and \lib{startup} libraries contain bindings {\C} code and the actual {\C} code compiled with \prog{gcc}.
...
Both figures make the distinction between the three sections reported by the \emph{size} program but only the \textbf{text} segment is prominent.
