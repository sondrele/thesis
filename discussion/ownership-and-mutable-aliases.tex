...
% However, this is an application-dependent issue, this might be the right decision for the low level library the emlib bindings are intended to be. An auxiliary library can then be defined to handle the issue in an opinionated fashion, which might apply to alot of application, but not all.
...
The problem that we have described throughout this section can be considered to be an application-dependent issue, for some programs it might be acceptable to send or receive garbled data at small rates, while it might be unacceptable for other applications.
...
Instead, we can define an auxiliary library which can be implemented to handle the issue in an opinionated fashion, that might apply to a lot of different applications, but not to all.
...
The library can then provide routines to either \emph{lock} the access to a peripheral during an interrupt, or provide \emph{channels} that can be used to safely communicate directly with the peripheral.
