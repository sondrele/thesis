...
The {\elf}sec{.text} section contains the program code and any read-only data defined in the code.
...
Copying the {\elf}sec{.data} section into \gls{ram} is handled by the \func{ResetHandler} function shown in \autoref{lst:reset-handler}
...
The \func{\_start} function finds the size of the {\elf}sec{.bss} section which describes the size of zero-initialized area in \gls{ram}.
