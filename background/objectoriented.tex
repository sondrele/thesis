...
The layout is aslo a implementation detail of Rust for the same reasons but annotating the struct with \texttt{\#[repr(C)]}, it will ensure that it is compatible with C's \gls{ffi}.
...
The function then uses the object reference in the same manner as the implicit \texttt{this} parameter in conventional object oriented languages such as Java.
...
We use the \texttt{impl} block to define the same behavior in Rust, but the methods are called with the dot notation known from object oriented languages.
...
The Rust code in \autoref{fig:oo:instantiate} contains the \texttt{unsafe} keyword.
...
Well, firstly the pointer can be \texttt{null}, this case is handled in \texttt{as\_mut} by returning \texttt{None}.
The next unsafe thing about dereferencing a raw pointer is if the pointer points into invalid memory.
For instance, dereferencing the pointer \texttt{0x} \todo{Find a funky address} causes the Gecko to HardFault on a Bus Error.
