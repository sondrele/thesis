...
The programmer is responsible for maintaining the safe invariants of the compiler when using a {\unsafe} block.
...
This also tells us that using an unsafe block is not the same as disabling the compiler, it is merely a mechanism for the programmer to take responsibility for the safety of part of the program in the cases where the compiler is unable to.
...
fn main() {
  let name = unsafe {
    ...
    // then the block is turned into a expression which
    // evaluates to value of this statement
