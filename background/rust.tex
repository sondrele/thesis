...
\rust's \core library exposes two pointer types which are considered \textit{unsafe} by the language, \autoref{ssub:unsafe_code} explains why.
...
A \rust library or executable is organized into a \emph{crate}, and such a crate can consist of many \emph{modules}.
...
By default, every \rust application is linked with the the \rust \std library, which \emph{re-exports} functionality from the standard libraries, like e.g. the \texttt{collections} crate.
