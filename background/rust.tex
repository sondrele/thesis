...
In order to achieve full memory safety, we have to remove all forms for memory leaks and dangling pointers to invalid memory.
...
% , this is usually the case when the variable goes out of scope and becomes unreachable in the program.
% This memory can safely be deallocated when there is no longer any need for it, i.e. the variable holding the handle to the memory goes out of scope.
...
A common implementation is a reference counted garbage collector \cite{Wilson1992} that keeps a count over the number of references to the variables and deallocates them when there are no valid references to them.
...
The variables' reference counts must be altered every time they enters and leaves a program scope.
...
The root set consists of global variables, local variables in the stack and any variable that may be accessible from the current state of the registers.
...
There are also many other techniques of garbage collection, that are often a variation of the ones already described.
