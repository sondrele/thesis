...
We can see from the example that \texttt{a} is defined to be a 32-bit integer with the value 5, and because the value \texttt{b} later gets assigned the \texttt{a}'s value, Rust will automatically infer \texttt{b} to be of the same type.
...
We can also see that both \texttt{Some} and \texttt{None}, i.e. all possible outcomes of the \texttt{Option}, is being matched against.
