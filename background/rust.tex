...
This section starts by describing {\rust}'s standard library and its type-system, before moving on to presenting some of the language features that makes {\rust} so different from {\C}.
...
The \gls{rsl} is included into all {\rust} programs and strives to be an abstraction layer to write portable applications.
The library is implemented as a facade, meaning that the crate {\std} provides a thin wrapper which reexports a stable interface for crates like \lib{alloc} (for memory allocation), \lib{collections} (general purpose data structures) and {\core} (described in the next section).
In addition to providing the facade, {\std} crate defines platform abstractions like files, network protocols and threading, all of which depends on an OS.
This basic fact renders the {\std} crate unusable in an embedded system as these dependencies will not be met.
Luckly it is possible to opt-out of using the library.

It is important to notice the distinction between the \gls{rsl} which includes the functionality provided by the underlying Collection, Allocation and Core libraries, and the {\std} crate which defines the facade and the mentioned additional abstractions.
...
The distinction between \gls{rcl} and the {\core} crate is not that clear as the {\std} and \gls{rsl} as \gls{rcl} does not export any functionallity which is not defined in {\core}.
...
An example of defining a \code{Book} is shown in \autoref{lst:book_struct}, anyone that is familiar with {\C} will see the similarity of the \keyword{struct} definition, but there are also a couple of extra things to notice from the example.
...
The \keyword{impl} keyword lets us implement member functions for the \code{Book}, similar to class-declarations in {\Cpp} and other object-oriented languages.
...
As mentioned earlier, {\rust} provides abstractions on top of its raw pointers, the \code{Box} is an example of this in action.
...
\autoref{lst:rust:list-iter} \todo{We should remove the clone() from this example} shows an example of how the \code{Iterator} trait can be implemented for the \code{List} type.
...
\autoref{lst:rust:for} shows a how a \code{for} loop is used to loop over a vector of numbers, this code is de-sugared by {\rust} into the equivalent (simplified) code shown in \autoref{lst:rust:desugared-for}.
...
\autoref{lst:closure:filter} shows how a closure can be used to filter a list of numbers, it also demonstrates one of the \code{Iterator} trait's many use cases.
The example initializes a vector with four numbers and makes an iterator by calling the \code{iter()} function.
Once the iterator is acquired, the \code{filter()} function is called with a closure as argument.
The expression \code{|&el| *el < limit} defines the closure, which takes an argument \code{el} and returns a \code{bool}.
The \code{limit} is a free variable which is found in the scope of the \texttt{main} function, this variable is part of the environment that is \emph{enclosed} by the closure.
...
We can see from this example that {\rust} requires us to annotate all references that are associated the function's arguments and return values with a name, in this case we have chosen the name \code{'a}.
The explicit name of the lifetime does not matter, what matters is that {\rust} forces us to allocate the book elsewhere, so that it is alive in the stackframe that encloses the function call to \code{get\_name\_as\_ref}.
