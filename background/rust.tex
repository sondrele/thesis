...
The \gls{rsl} is the is included into all {\rust} programs and strives to be an abstraction layer to write portable applications.
...
In addition to providing the facade, {\std} crate defines platform abstractions like files, network protocols and threading, all of these assumes an OS.
This basic fact renders the {\std} crate unusable in a barebone embedded system as these dependencies will not be met.
Luckly it is possible to opt-out of using the library.

Notice the distinction between the \gls{rsl} which includes the functionality provided by the underlying Collection, Allocation and Core libraries, and the {\std} crate which defines the facade and the mentioned additional abstractions.
