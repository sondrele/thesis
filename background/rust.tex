...
However, we can end up with dangling pointers if we happen to have multiple references into our vector in {\Cpp}.
This is the case because, when the vector grows and gets reallocated, references into the vector will not be updated to the newly allocated storage but will instead be dangling pointers into the now old and deallocated storage.
