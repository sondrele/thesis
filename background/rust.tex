...
One of {\rust}'s most attractable feature is the ability to maintain full memory safety without sacrificing performance.
...
\item [Lifetime] \hfill \\
  ...
  It always corresponds to some expression or block within the program \cite{web:rust_lifetime_guide}.
...
{\rust}'s ownership system is one of its zero-cost abstractions, since all borrow-checking and ownership- and lifetime-analysis are done statically during compilation.
In this respect {\rust} is interesting to consider in an embedded system as it can provide memory management without the runtime cost of a garbage collector.
...
The {\rust} compiler will assign a \emph{lifetime} to every value in an program.
This lifetime is used by the compiler to prevent problems like \emph{use-after-write} by failing the compilation process if such errors are discovered.
The lifetimes are, for many uses cases, inserted automaticaly by the compiler.
This is done with a process called lifetime \emph{elision}.
In \autoref{lst:block-lifetime} we see the lifetime annotated in the comments.
...
}               // Owning handle `a' goes out of scope and deallocates value 42.
  \end{minted}
  \caption{Block Lifetime}
  \label{lst:block-lifetime}
\end{listing}

Let us consider a function with a reference as a parameter as shown in \autoref{lst:func-param-lifetime}.
In the comment on line 1 we have included the explicit \emph{lifetime specifier} as it would be inserted by the compiler.

\begin{listing}[H]
  \begin{minted}{rust}
// foo<'a>(x: &'a u32) { // lifetime specifier by compiler
fn foo(x: &u32) {
...
fn main() {
  ...
  \end{minted}
  \caption{Function Parameter Lifetime}
  \label{lst:func-param-lifetime}
\end{listing}

For a complete introduction to lifetimes we refer the reader to the lifetime chapter of the Rust book \cite{web:rust_book}.
