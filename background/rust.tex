...
The facede provides a thin wrapper which reexports a stable interface for libraries like \lib{alloc} (for memory allocation), \lib{collections} (general purpose data structures), and {\core} (described in the next section).
...
The \gls{rsl} encompasses the standard library as a whole, i.e. \lib{alloc} is a part of \gls{rsl}.
On the other hand, {\std} is just the actual software component, i.e. \lib{alloc} is not a part of {\std}, even though it is reexported by {\std}.
...
\autoref{lst:rust:option} shows the definition of \code{Option}, taken from the standard library.
...
If the variable is \code{Some}, the definition also says that it needs to contain a value of type \code{T}.
This \code{T} can be anything and is how {\rust} defines a generic type.
The \code{Option} type \emph{can} be used to encode the same programming pattern as \emph{pointers with a sentinel value of 0} (e.g. 0 for representing no value or the end of a datastructure), but in a fundamentaly safer way.
...
In order to use a potential pointer which is contained within a \code{Option}, the programmer \emph{must} unwrap the value and explicitly handle the case when the \code{Option} holds the value \code{None}.
If it holds the value \code{Some}, the programmer is guaranteed by {\rust}'s memory safety guarantees, discussed later in this section, that the pointer is valid and safe to use.
Even though the \code{Option} can be used analogous to a {\C} pointer, it is important to note their differences.
It acts more like a wrapper around a potential value, where its internal value is not \emph{directly} accessible, which forces the programmer to handle the cases where it can be \code{None}.
...
\code{Slices} like the arrays comes with bounds checking, although this can be circumvented for performance reasons.
...
Slice syntax is denoted by \code{\&[T]}, which reads like `a slice of a finite array with type \code{T}.'
...
This makes {\rust} interesting to consider in a bare-metal system, as it can provide memory management without the runtime cost of a garbage collector.
...
Modern programming languages, like {\Java} and {\lang{Python}}, typically achieves memory safety with a \emph{garbage collector} that runs while the program is being executed with the sole purpose of deallocating unused memory.
...
\subsubsection{Memory Management with Ownership}
...
Multiple \emph{references} can exist to the same place in memory, as long as there is only one \concept{owner} of the actual handle, and that the \concept{owner} does not deallocate the memory before all borrowers are finished.
...
In the comment on line 1 we have included the explicit \emph{lifetime specifier} (\code{'a}) as it would be inserted by the compiler.
