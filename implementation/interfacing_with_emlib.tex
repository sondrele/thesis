...
\prog{light\_sense}  &
Uses the {\chip{STK}}'s light sensor to measure the light intensity, and ligts a LED when the intensity goes below a threshold &
...
Every external clock is disabled by default \todo{Is this true? Is every clock disabled?} in order for the peripherals not to draw any power.
By enabling the clock, the \gls{mcu} will get access to control it.
...
The next lines enable interrupts for the Timer, and the NVIC interrupt vector is set up to call the function shown in \autoref{lst:timer_interrupt_handler} every time an interrupt it triggered by the Timer.
