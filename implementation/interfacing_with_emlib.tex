...
As with \texttt{emlib}, it was not a goal to fully support all available peripheral drivers that are available in \texttt{emdrv}.
...
The project includes two examples that demonstrate how to use the \texttt{flash} driver, they are summarized in \autoref{tab:emdrv_bindings}.
...
The Driver is used by the \texttt{sensor-tracker}. \\
...
\texttt{i2c}  &
This driver is used by the \texttt{sensor-tracker} application, it exposes functions initialize normal \gls{i2c} data transfers. \\

\texttt{tft}  &
This binding exposes one function to initialize the TFT screen on the \texttt{DK}.
It is used in the \texttt{circle-game}. \\

    \hline
  \end{tabular}

  \caption{\texttt{emdrv} driver bindings.}
...
\texttt{flash} &
Demonstrates reading and writing to the \texttt{STK}'s flash memory. &
flash-driver, \gls{gpio} \\

\texttt{light\_measure} &
Based on the \texttt{light\_sense} example from \autoref{tab:emlib_examples}.
The results from measuring the light intensity is saved to flash.
When the user presses a button the \texttt{STK} will initiate a transfer that reeds all the data stored to flash and transmits it over a \gls{usart} connection. &
...
\todo{Det er vel en seksjon som beskriver dette? Vi får fylle ut mer her etter hvert som vi får oversikt}
