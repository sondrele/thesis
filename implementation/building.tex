...
The compilation process consisted of compiling the \file{blinky.rs} file to assembly by passing the target architecture and assembly output flag to the {\rustc} compiler.
...
The build system was then evolved to include the \gls{RCL} library by building it manually for the architecture producing a \file{libcore.rlib} library file which was linked to the \file{blinky.rs} file before compiling it to assembly.
The initial bindings to emlib was created and compiled together with the startup and system files.

The final revision on the build system using Makefiles was to eliminate the \prog{gcc} build step for the Rust files.
This revision utilizes the \cmd{link-args} argument to the {\rustc} compiler to specify the arguments to the linker.
...
Later this was separated out to produce an \file{libemlib.rlib} which complements an \file{libemlib.a} containing the C porting of the emlib code.
By doing this the system and startup code was separated to \file{libstartup.rlib} and \file{libstartup.a}.
...
The \lib{modules} library is a high level library over the {\emlib} and \lib{emdrv} libraries.
The reset of the libraries provides the Rust bindings in the \format{*.rlib} part and the C implementations in the \format{*.a} portion.
