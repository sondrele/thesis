...
The components listed in \autoref{tab:build:components} were identified as the minimal set of needed to build an executable for the Gecko.

\begin{table}[H]
  \centering
  \begin{tabular}{l l l}
    thumbv7m-none-eabi.json & llvm spec for compiler backend \\ \hline
    zero.rs & minimal runtime requirements \\ \hline
    blinky.rs & a program to execute \\ \hline
    efm32gg.ld & linker script \\ \hline
    startup\_efm32gg.s & ResetHandler and interrupt vector \\ \hline
    system\_efm32gg.c & System Clock management \\
  \end{tabular}
  \caption{Initial Components for building Rust for ARM Cortex-M}
  \label{tab:build:components}
\end{table}

The components in \autoref{tab:build:components} are the files included in the initial successfull build for the Gecko located at geckoboot.rs \cite{web:geckobook-initial} for references.
In order to compile these components into an executable a Makefile was developed.
The compilation process consisted of compiling the \texttt{blinky.rs} file to assembly by passing the target architecture and assembly output flag to the \texttt{rustc} compiler.
This file was then, along with the startup and system files compiled into object files with \texttt{arm-none-eabi-gcc} and linked into an executable with the linker script using \texttt{arm-none-eabi-ld}.

Note: The initial build did not contain the \gls{RCL}.

The build system was then evolved to include the \gls{RCL} library by building it manually for the architecture producing a \texttt{libcore.rlib} library file which was linked to the \texttt{blinky.rs} file before compiling it to assembly.
The initial bindings to emlib was created and compiled together with the startup and system files.

The final revision on the build system using Makefiles was to eliminate the \texttt{gcc} build step for the Rust files.
This revision utilizes the \texttt{link-args} argument to the \texttt{rustc} compiler to specify the arguments to the linker.
...
\begin{table}[H]
  ...
  \begin{tabular}{l l l l}
    ...
    &&&\\
    1. & Building emlib+system+startup & \texttt{libcompiler-rt.a} & static archive \\
    2. & Building \gls{RCL} & \texttt{libcore.rlib} & static Rust crate \\
    3. & Building and linking executable & \texttt{\%.bin} & executable elf file \\
  \end{tabular}
  \caption{Build steps}
  \label{tab:build-steps}
\end{table}

At this stage of the evolution of the build system the emlib bindings was buils as part of the Rust application code.
Later this was separated out to produce an \text{libemlib.rlib} which complements an \texttt{libemlib.a} containing the C porting of the emlib code.
By doing this the system and startup code was separated to \texttt{libstartup.rlib} and \texttt{libstartup.a}.
...
\todo{Reference section explaining these flags in detail?}
...
The problem with resolving the location of generated metadata were solved implicitly just by utilizing Cargo, and the extra linker-arguments could easily be passed on to the invocation of \cmd{cargo-linkargs} via the project's makefile.
...
There have been an issue on Cargo's project page about implementing a new subcommand (called \cmd{cargo-rustc}) for the package manager.
...
The collection of the output file for the library is presented in \autoref{fig:lib:structure}.
...
The figure shows that all of the libraries except for \texttt{modules} consists of both a C static archive (\texttt{*.a}) and a Rust library (\texttt{*.rlib}).
The \texttt{modules} library is a high level library over the \texttt{emlib} and \texttt{emdrv} libraries.
The reset of the libraries provides the Rust bindings in the \texttt{*.rlib} part and the C implementations in the \texttt{*.a} portion.
