...
This includes managing project dependencies, and building \gls{rel} and {\emlib} for the ARM architecture.
...
Throughout this thesis, we have worked on many different project ideas that depends on the work that is presented in the previous chapters.
A collection of the more interesting of these projects is presented in this chapter.
First, we describe a library that was ported from {\C} to {\rust}, this shows how {\rust}'s concept of \emph{unsafe} can guide the programmer to write more secure code.
In \autoref{sec:irq-closures}, we describe a library that provides a {\rust}-idiomatic way to handle interrupts, which is motivated by wanting to provide a safer way of interacting with the \gls{mcu} peripherals.
...
In the last section, we describe the two applications which was developed for evaluating the {\rg} platform.
}
